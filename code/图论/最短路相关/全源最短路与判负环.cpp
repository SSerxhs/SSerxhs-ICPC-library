...
int main()
{
	...
	while (1)
	{
		...
		for (k = 0; k < n; k++) for (i = 0; i < n; i++) if (dis[i][k] < oo) for (j = 0; j < n; j++) if (dis[k][j] < oo)
			dis[i][j] = max(min(dis[i][j], dis[i][k] + dis[k][j]), -inf * 2);
		for (k = 0; k < n; k++) if (dis[k][k] < 0) for (i = 0; i < n; i++) if (dis[i][k] < oo) for (j = 0; j < n; j++) if (dis[k][j] < oo) ne[i][j] = 1;
		while (q--)
		{
			int u, v;
			cin >> u >> v;
