...
\verb|lower_bound| 要求前缀和单调，常见用法是所有单点值非负。
...
题意/接口：$\sum\limits_{x=0}^{n}\lfloor \dfrac {ax+b}m\rfloor$ （$0\le a,b<m$）。
...
第一版要求 \verb|merge(x,y,W{})| 表示同一父结点坐标系下合并；若合并语义更复杂，使用合并版。
...
\verb|find_cycle| 若不是弦图，返回一个无弦环（首尾相连但不重复首点）；若是弦图返回空。
...
\lstinputlisting{code/语言基础/Makefile}
\subsection{debug.h}
