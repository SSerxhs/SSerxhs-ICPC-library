...
常见的被漏掉的初始代码：
\lstinputlisting{templates/前言/前言.cpp}
...
\lstinputlisting{templates/数据结构/树状数组.cpp}
...
\lstinputlisting{templates/数据结构/线段树.cpp}

\lstinputlisting{templates/数据结构/线段树_2.cpp}
...
\lstinputlisting{templates/数据结构/珂朵莉树.cpp}
...
\lstinputlisting{templates/数据结构/带删堆.cpp}
\subsection{前 k 大的和}
...
\lstinputlisting{templates/数据结构/前 k 大的和.cpp}
...
\lstinputlisting{templates/数据结构/左偏树_可并堆.cpp}
...
\lstinputlisting{templates/数据结构/树状数组区间加区间求和.cpp}
...
\lstinputlisting{templates/数据结构/二维树状数组矩形加矩形求和.cpp}
...
\lstinputlisting{templates/数据结构/带修莫队.cpp}
...
\lstinputlisting{templates/数据结构/二次离线莫队.cpp}
...
\lstinputlisting{templates/数据结构/回滚莫队.cpp}
...
\lstinputlisting{templates/数据结构/李超树.cpp}

\subsection{李超树（动态开点）}
\lstinputlisting{templates/数据结构/李超树(动态开点).cpp}
...
\lstinputlisting{templates/数据结构/线性基.cpp}
...
\lstinputlisting{templates/数据结构/splay.cpp}
...
\lstinputlisting{templates/数据结构/fhq-treap.cpp}
...
\lstinputlisting{templates/数据结构/笛卡尔树的线性建树.cpp}
...
\lstinputlisting{templates/数据结构/扫描线.cpp}
...
\end{enumerate}
其中 $B_i$ 是 $A_i$ 的历史最大值。
\lstinputlisting{templates/数据结构/Segmenttree Beats!.cpp}

\subsection{$k$-d 树（二进制分组）}

均摊 $O(\log ^2n)$ 插入，$O(\sqrt n)$ 矩形查询。

\lstinputlisting{templates/数据结构/k-d 树(二进制分组).cpp}

\subsection{双端队列全局查询}

对一个支持结合律的信息 T，维护 \verb|deque| 内信息的和。总复杂度线性。

\lstinputlisting{templates/数据结构/双端队列全局查询.cpp}

\subsection{静态矩形加矩形和}

\lstinputlisting{templates/数据结构/静态矩形加矩形和.cpp}

\subsection{线段树分裂}

\lstinputlisting{templates/数据结构/线段树分裂.cpp}

\subsection{手写 bitset}

\lstinputlisting{templates/数据结构/手写 bitset.cpp}

\subsection{区间众数}

\lstinputlisting{templates/数据结构/区间众数.cpp}
...
\lstinputlisting{templates/数据结构/表达式树.cpp}
...
\lstinputlisting{templates/数据结构/区间排序区间复合.cpp}
\newpage
...
\lstinputlisting{templates/数学/矩阵类(较新).cpp}
...
\lstinputlisting{templates/数学/在线 O(1) 逆元.cpp}

\subsection{Strassen 矩阵乘法}

没用，不如卡常。$O(n^{\log_27})$。

\lstinputlisting{templates/数学/Strassen 矩阵乘法.cpp}
...
\lstinputlisting{templates/数学/扩展欧拉定理.cpp}
...
\lstinputlisting{templates/数学/exgcd.cpp}

递推版：

\lstinputlisting{templates/数学/exgcd_2.cpp}
...
\lstinputlisting{templates/数学/exCRT.cpp}
...
\lstinputlisting{templates/数学/exBSGS.cpp}

\subsection{exLucas}

求组合数。

\lstinputlisting{templates/数学/exLucas.cpp}
...
\lstinputlisting{templates/数学/杜教筛.cpp}
...
\lstinputlisting{templates/数学/mu^2(n) 前缀和.cpp}
\subsection{线性规划}
用法：构造函数指明目标函数系数，\verb|add| 函数增加限制。额外的限制是 $x_i\ge 0$。
\lstinputlisting{templates/数学/线性规划.cpp}
...
\lstinputlisting{templates/数学/线性插值(k 次幂和).cpp}
\subsection{幂乘等比}
...
\lstinputlisting{templates/数学/幂乘等比.cpp}
...
\lstinputlisting{templates/数学/原根.cpp}

\subsection{筛全部原根}

\lstinputlisting{templates/数学/筛全部原根.cpp}

\subsection{高斯消元（浮点数）}

$O(n^3)$，$O(n^2)$。

\lstinputlisting{templates/数学/高斯消元(浮点数).cpp}
...
\lstinputlisting{templates/数学/行列式求值(任意模数).cpp}
...
\lstinputlisting{templates/数学/BM_稀疏矩阵系列.cpp}
...
\lstinputlisting{templates/数学/光速乘.cpp}

\subsection{二次剩余}

\lstinputlisting{templates/数学/二次剩余.cpp}

\subsection{$k$ 次剩余}
\lstinputlisting{templates/数学/k 次剩余.cpp}

网上的超快版本

\lstinputlisting{templates/数学/k 次剩余_2.cpp}

\subsection{FWT/子集卷积}

$O(n2^n)$，$O(2^n)$。注意全都是无符号的。

\lstinputlisting{templates/数学/FWT_子集卷积.cpp}
...
\lstinputlisting{templates/数学/NTT.cpp}
...
\lstinputlisting{templates/数学/MTT.cpp}
\subsection{FFT}

\lstinputlisting{templates/数学/FFT.cpp}

\subsection{约数个数和}

$O(\sqrt[3]n\log n)$。

\lstinputlisting{templates/数学/约数个数和.cpp}
...
\lstinputlisting{templates/数学/万能欧几里得_min of mod of linear.cpp}
...
\lstinputlisting{templates/数学/高斯整数类.cpp}
...
\lstinputlisting{templates/数学/Miller Rabin_Pollard Rho.cpp}

\lstinputlisting{templates/数学/Miller Rabin_Pollard Rho_2.cpp}
...
\lstinputlisting{templates/字符串/字典树(trie 树).cpp}
...
\lstinputlisting{templates/字符串/AC 自动机.cpp}
...
\lstinputlisting{templates/字符串/hash.cpp}
...
\lstinputlisting{templates/字符串/hash_2.cpp}
...
\lstinputlisting{templates/字符串/KMP.cpp}
...
\lstinputlisting{templates/字符串/manacher.cpp}
...
\lstinputlisting{templates/字符串/SA.cpp}

\subsection{SAM}

$O(n\sum)$，$O(2n\sum )$。

\lstinputlisting{templates/字符串/SAM.cpp}

\subsection{ukkonen 后缀树}

$O(n)$，$O(2n\sum )$。

\lstinputlisting{templates/字符串/ukkonen 后缀树.cpp}

\subsection{ukkonen 后缀树（重构）}

\lstinputlisting{templates/字符串/ukkonen 后缀树(重构).cpp}
...
\lstinputlisting{templates/字符串/Z 函数.cpp}
...
\lstinputlisting{templates/字符串/最小表示法.cpp}
...
\lstinputlisting{templates/字符串/带通配符的字符串匹配.cpp}

快一些的版本，手动拆开了多项式乘法。

\lstinputlisting{templates/字符串/带通配符的字符串匹配_2.cpp}
\newpage
...
\lstinputlisting{templates/图论/最小生成树相关/切比雪夫距离最小生成树.cpp}
...
\lstinputlisting{templates/图论/最小生成树相关/最小乘积生成树.cpp}
...
\lstinputlisting{templates/图论/最小生成树相关/最小斯坦纳树.cpp}
...
\lstinputlisting{templates/图论/最短路相关/全源最短路与判负环.cpp}
...
\lstinputlisting{templates/图论/最短路相关/Dijkstra_SPFA_Johnson.cpp}
...
\lstinputlisting{templates/图论/最短路相关/无向图最小环.cpp}

\subsubsection{输出负环}

\lstinputlisting{templates/图论/最短路相关/输出负环.cpp}
...
\lstinputlisting{templates/图论/二分图与网络流建图/二分图最小点集覆盖.cpp}
...
\lstinputlisting{templates/图论/二分图与网络流建图/有向无环图最小不相交链覆盖.cpp}
...
\lstinputlisting{templates/图论/匹配与网络流相关代码/二分图最大权匹配.cpp}

\subsubsection{一般图最大匹配}

\lstinputlisting{templates/图论/匹配与网络流相关代码/一般图最大匹配.cpp}

\subsubsection{一般图最大权匹配}

$n=400$：UOJ 600ms, Luogu 135ms

\lstinputlisting{templates/图论/匹配与网络流相关代码/一般图最大权匹配.cpp}
...
\lstinputlisting{templates/图论/匹配与网络流相关代码/网络流.cpp}
...
\lstinputlisting{templates/图论/匹配与网络流相关代码/假带花树.cpp}
...
\lstinputlisting{templates/图论/匹配与网络流相关代码/Stoer-Wagner 全局最小割.cpp}
...
\lstinputlisting{templates/图论/匹配与网络流相关代码/最小割树.cpp}
...
\lstinputlisting{templates/图论/缩点相关/双极分解.cpp}
...
\lstinputlisting{templates/图论/缩点相关/点双.cpp}
...
\lstinputlisting{templates/图论/缩点相关/边双.cpp}
...
\lstinputlisting{templates/图论/缩点相关/Tarjan 强连通分量.cpp}

\subsubsection{动态强连通分量}

给出一个加边序列，\verb|solve| 会返回每个时间进入强连通分量的边。点标号范围是 $[0,n)$

\lstinputlisting{templates/图论/缩点相关/动态强连通分量.cpp}
...
\lstinputlisting{templates/图论/缩点相关/圆方树.cpp}

\subsubsection{广义圆方树}

建议使用点双来做这个。你只需要对每个点双建一个虚点，向点双内所有原点连边，就得到了广义圆方树。

\lstinputlisting{templates/图论/缩点相关/广义圆方树.cpp}
...
\lstinputlisting{templates/图论/缩点相关/2-sat.cpp}
...
\lstinputlisting{templates/图论/缩点相关/Kosaraju 强连通分量(bitset 优化).cpp}
...
\lstinputlisting{templates/图论/树上问题/换根 dp.cpp}

合并版：

\lstinputlisting{templates/图论/树上问题/换根 dp_2.cpp}
...
\lstinputlisting{templates/图论/树上问题/轻重链剖分_DFS 序 LCA.cpp}
...
\lstinputlisting{templates/图论/树上问题/换根树剖.cpp}
$O(n+q\log n)$，$O(n)$。
...
\lstinputlisting{templates/图论/树上问题/毛毛虫剖分.cpp}
...
\lstinputlisting{templates/图论/树上问题/树上启发式合并，DSU on tree.cpp}

\subsubsection{长链剖分（$k$ 级祖先）}

$O(n\log n+q)$，$O(n)$。

\lstinputlisting{templates/图论/树上问题/长链剖分(k 级祖先).cpp}
...
\lstinputlisting{templates/图论/树上问题/长链剖分(dp 合并).cpp}
\subsubsection{LCT}
...
\lstinputlisting{templates/图论/树上问题/LCT.cpp}
...
\lstinputlisting{templates/图论/树上问题/带子树的 LCT.cpp}
...
\lstinputlisting{templates/图论/树上问题/动态 dp(全局平衡二叉树).cpp}

\subsubsection{全局平衡二叉树（修改版）}

$O((n+q)\log n)$，$O(n)$。

\lstinputlisting{templates/图论/树上问题/全局平衡二叉树(修改版).cpp}
...
\lstinputlisting{templates/图论/树上问题/虚树.cpp}
...
\lstinputlisting{templates/图论/树上问题/点分治.cpp}
...
\lstinputlisting{templates/图论/树上问题/点分树.cpp}

圆环修改和单点查询：

\lstinputlisting{templates/图论/树上问题/点分树_2.cpp}

单点修改和圆环查询：

\lstinputlisting{templates/图论/树上问题/点分树_3.cpp}

\subsubsection{（基环）树哈希}

有根树返回每个子树的哈希值，无根树返回树的哈希值（长度至多为 $2$ 的 \verb|vector|），基环树返回图的哈希值（长度等于环长的 \verb|vector|）。

\lstinputlisting{templates/图论/树上问题/(基环)树哈希.cpp}

\subsection{欧拉路相关}

\subsubsection{构造：字典序最小}

\lstinputlisting{templates/图论/欧拉路相关/构造：字典序最小.cpp}

\subsubsection{回路/通路构造}

$O(n+m)$，$O(n+m)$。

\lstinputlisting{templates/图论/欧拉路相关/回路_通路构造.cpp}
...
\lstinputlisting{templates/图论/欧拉路相关/回路计数(BEST 定理)_生成树计数.cpp}
...
\lstinputlisting{templates/图论/三_四元环计数.cpp}
...
\lstinputlisting{templates/图论/支配树.cpp}

一般图：标号从 $1$ 开始。

\lstinputlisting{templates/图论/支配树_2.cpp}

\subsection{prufer 与树的互相转化}

$O(n)$，$O(n)$。

\lstinputlisting{templates/图论/prufer 与树的互相转化.cpp}
...
\lstinputlisting{templates/图论/最小密度环.cpp}
...
\lstinputlisting{templates/图论/点染色.cpp}

\subsection{最大独立集}

爆搜。

\lstinputlisting{templates/图论/最大独立集.cpp}
...
\lstinputlisting{templates/图论/弦图.cpp}
...
\lstinputlisting{templates/计算几何/自适应 simpson 法.cpp}

\subsection{计算几何全}

功能其实比较少，因为实际遇到的几何题不多。最有用的可能是闵可夫斯基和合并凸包，和常规的线段判交之类的。其余功能最好直接使用 HDU 板。

\lstinputlisting{templates/计算几何/计算几何全.cpp}

另附一份支持二分斜率的代码。这里维护的是 $(x,y)$，其中 $y=C+x^2$，因此 $x$ 整体增加会影响 $y$。
\lstinputlisting{templates/计算几何/计算几何全_2.cpp}
\newpage
...
\lstinputlisting{templates/杂项/枚举大小为 k 的集合.cpp}
...
\lstinputlisting{templates/杂项/min plus 卷积.cpp}

\subsection{所有区间 GCD}

需要自定义 \verb|fun|，如 $\gcd$，$\operatorname{and}$，$\operatorname{or}$。

\lstinputlisting{templates/杂项/所有区间 GCD.cpp}
\subsection{整体二分（区间 $k$-th）}

$O((n+q)\log a)$，$O(n+q)$。

\lstinputlisting{templates/杂项/整体二分(区间 k-th).cpp}

\subsection{高精度}

除法和取模有点问题，但 gcd 是对的。

\lstinputlisting{templates/杂项/高精度.cpp}
...
\lstinputlisting{templates/杂项/分散层叠算法(Fractional Cascading).cpp}
...
\lstinputlisting{templates/杂项/圆上整点(二平方和定理).cpp}

\subsection{Stern-Brocot Tree}

你需要时刻保持传入的 $\dfrac{num}{den}$ 满足 $\gcd(num,den)=1$。

\lstinputlisting{templates/杂项/Stern-Brocot Tree.cpp}

\subsection{快速取模}

\lstinputlisting{templates/杂项/快速取模.cpp}

\subsection{IO 优化}

\lstinputlisting{templates/杂项/IO 优化.cpp}
\subsection{手动开栈}

一种写法是文件开头放，但部分 OJ 会失效。

\lstinputlisting{templates/杂项/手动开栈.cpp}

另一种写法是在 \verb|main| 开头写，但必须以 \verb|exit(0)| 结束程序。

以下两个应该有一个是对的，不对会 CE。

\lstinputlisting{templates/杂项/手动开栈_2.cpp}

\subsection{德扑}

\verb|solve| 返回按照出现次数排序的 \verb|vector<int>|（$0$ 下标处为牌型），这样就可以字典序比较了。

\lstinputlisting{templates/杂项/德扑.cpp}

\subsection{随机函数}

\verb|u64| $\to$ \verb|u64|

\lstinputlisting{templates/杂项/随机函数.cpp}
...
\lstinputlisting{templates/语言基础/Makefile.cpp}
\subsection{debug.h}

\lstinputlisting{templates/语言基础/debug.h.cpp}

\subsection{初始代码}

\lstinputlisting{templates/语言基础/初始代码.cpp}

抄写完 \verb|debug.h| 后，补充于 \verb|using namespace std;| 下一行：

\lstinputlisting{templates/语言基础/初始代码_2.cpp}


\subsection{bitset}

\lstinputlisting{templates/语言基础/bitset.cpp}
...
\lstinputlisting{templates/语言基础/pb__ds 和一些奇怪的用法.cpp}
...
\lstinputlisting[language=python]{templates/语言基础/python 使用方法.py}


类的构造，重载运算符
\lstinputlisting[language=python]{templates/语言基础/python 使用方法_2.py}

初始代码

\lstinputlisting[language=python]{templates/语言基础/python 使用方法_3.py}
...
\lstinputlisting{templates/其他人的板子(补充)/最小树形图.cpp}
\subsection{MTT+exp}

\lstinputlisting{templates/其他人的板子(补充)/MTT+exp.cpp}

\subsection{半平面交}

\lstinputlisting{templates/其他人的板子(补充)/半平面交.cpp}

\subsection{多项式复合 (yurzhang)}

$O(n\log n\sqrt{n\log n})$，奇慢无比，慎用

\lstinputlisting{templates/其他人的板子(补充)/多项式复合 (yurzhang).cpp}

\subsection{下降幂多项式乘法}

$O(n\log n)$。

\lstinputlisting{templates/其他人的板子(补充)/下降幂多项式乘法.cpp}

\subsection{弦图找错}

\lstinputlisting{templates/其他人的板子(补充)/弦图找错.cpp}

\subsection{最长公共子序列}

复杂度 $O(\frac{nm}{\omega})$。

\lstinputlisting{templates/其他人的板子(补充)/最长公共子序列.cpp}

另一个实现

\lstinputlisting{templates/其他人的板子(补充)/最长公共子序列_2.cpp}
...
\lstinputlisting{templates/其他人的板子(补充)/区间 LIS(排列).cpp}


离线：

\lstinputlisting{templates/其他人的板子(补充)/区间 LIS(排列)_2.cpp}

\subsection{区间 LCS}

$s_{[0,a)}$ 和 $t_{[b,c)}$ 的 LCS

\lstinputlisting{templates/其他人的板子(补充)/区间 LCS.cpp}
\end{document}
