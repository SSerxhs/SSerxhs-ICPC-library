...
\begin{lstlisting}
ll triple(const vector<pair<int,int>> &edges)//0-labelled
{
	int n=0,i;
	...
	for (auto [u,v]:edges)
	{
		if (pair{d[u],u}>pair{d[v],v}) swap(u,v);
		e[u].push_back(v);
	}
	fill(all(d),0);
	ll ans=0;
	...
	{
		for (int u:e[i]) d[u]=1;
		for (int u:e[i]) for (int v:e[u]) ans+=d[v];
		for (int u:e[i]) d[u]=0;
	}
	return ans;
}
ll quadruple(const vector<pair<int,int>> &edges)//0-labelled
{
	int n=0,i;
	for (auto [u,v]:edges) n=max({n,u,v});
	++n;
	vector d(n,0),id(n,0),rk(n,0);
	...
	iota(all(id),0);
	stable_sort(all(id),[&](int u,int v) { return d[u]<d[v]; });
	...
	for (i=0; i<n; i++)
	{
		for (int u:lk[i]) for (int v:e[u]) if (rk[v]>rk[i]) ans+=d[v]++;
		for (int u:lk[i]) for (int v:e[u]) d[v]=0;
	}
	return ans;
}
\end{lstlisting}
