...
核心：构造 $g$ 满足 $h(n)=\sum\limits_{d|n}f(d)g(\dfrac{n}{d})$ 容易计算，
...
\begin{lstlisting}
int f(int *a,int n,int m)//这种写法不包含0处取值，n是值，m-1是次数，至少需要 m 项。换而言之，f(1,2,...,m)=a[1],a[2],...,a[m]
{
	if (n<=m) return a[n];
...
namespace sh
{
	...
	struct str
	{
		...
		vector<pa> a;
		vector<ll> s;
		template<class T> str(const vector<T> &_):n(_.size()),a(n+1),s(n)
		{
			a[0]={0,0};
			for (i=0; i<n; i++) a[i+1]=fma(a[i],b,s[i]=_[i]);
		}
		template<class T> str(const basic_string<T> &_):n(_.size()), a(n+1), s(n)
		{
			a[0]={0, 0};
			for (i=0; i<n; i++) a[i+1]=fma(a[i], b, s[i]=_[i]);
		}
...
时间复杂度为 $O(n^{\dfrac{1}{4}}+f(n))$，其中 $O(n^{\dfrac{1}{4}})$ 是 pollard-rho 的复杂度。

$f(n)$ 的量级不好分析，但不会超过约数个数 $O(d(n))\approx O(n^{\dfrac{1}{3}})$，且可以推测不能达到。
...
int main()
{
	...
	//it=s2.lower_bound(35);template<class T> vector<vector<T>> min_cut(int n, const vector<tuple<int, int, T>> &edges)//[0,n)
	{
		int m=edges.size(), i, s, t, cnt=0;
		vector<int> fir(n, -1), nxt(m*2, -1), fc(n), q(n);
		vector<pair<int, T>> e(m*2);
