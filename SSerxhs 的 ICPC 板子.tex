...
namespace chtholly_tree
{
	...
	public:
		...
		iter find(int x) const {return prev(upper_bound({x}));}
	};
}
...
\begin{lstlisting}
	template<class T,int M=sizeof(T)*8> struct base
	{
		array<T,M> a;
		base():a{ } { }
		bool insert(T x)
		...
		base operator+(base o) const { return o+=*this; }
	...
	template<class T=ll,int M=sizeof(T)*8> struct rangebase//[0,...)
	{
		...
		rangebase():a{{ }} { }
		rangebase(const vector<T> &b):a{{ }} { for (T x:b) insert(x); }
		void insert(T x)
		{
			int n=a.size()-1;
			a.push_back(a.back());
