...
查询第 $k$ 大/小，即 $k\text{-th}\min\limits_{\varnothing\not=T\subset S}r\oplus\bigoplus\limits_{x\in T} x$。

不允许选择空集，如果允许只需要将 \verb|con| 设置为 $1$。
...
#define all(x) (x).begin(),(x).end()
	...
	struct circle
	{
		...
		circle(const vector<vec<ll>> &b)
		{
			...
			int n = a.size(), i, j, k;
			for (i = 0; i < a.size(); i++) a[i] = (vec<db>)b[i];
			*this = circle(a);
		}
		tmpl bool cover(const vec<T> &a) { return sgn(dis((vec<db>)a, o) - r) <= 0; }
	};
	tmpl struct segment
	{
		vec<T> a, b;
...
using geo::eps, geo::pi, geo::segment, geo::read, geo::sgn, geo::dynamic_convex;
using Q = vec<ll>;
\end{lstlisting}
