...
namespace StoerWagner
{
	...
	template<typename TT> pair<T,vector<int>> mincut(int n,const vector<tuple<int,int,TT>> &edges)//[1,n]，返回某一集合
	{
...
支持添加一个条件 add(u,x,v,y)，表示 $a_u=x\Rightarrow a_v=y$。支持设定一个变量的值。
