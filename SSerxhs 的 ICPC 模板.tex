...
包含标记的线段树，支持线段树上二分。但只支持求左侧第一个符合条件的下标。
...
\begin{lstlisting}
template<class T> void min_order(T *a,int n)//[0,n)
{
	int i,j,k;
	T x,y;
	i=k=0;j=1;
...
你需要实现的是 \verb|sol1|，即按照 \verb|val| 为权值的答案。
...
功能其实比较少，因为实际遇到的几何题不多。最有用的可能是闵可夫斯基和合并凸包，和常规的线段判交之类的。
